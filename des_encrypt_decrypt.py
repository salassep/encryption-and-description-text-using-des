...
def invers_initial_permutation(ri_li):
  invers_initial_permutation_arr = [40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31, 38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13, 53, 21, 61, 29, 36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27, 34, 2, 42, 10, 50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25]
  invers_ip = []
  for x in invers_initial_permutation_arr:
    invers_ip.append(ri_li[x-1])
  return invers_ip

hex_str = input("e for encyrption, d for descryption : ")

if hex_str == 'e':
  
  binary_str = input("Input your binary plaintext : ")
  iter = int(input("How many iterations do you want : "))
  for i in range(iter):
    key = input("Input your binary key {} : ".format(i + 1))
    key_arr.append(key)
  ...
  for x in range(len(key_arr)):

    print("ITERATIONG ", x + 1)

    expansion_res = ekspansion_table(curr_ri)
    print("\nExpansion Phase : ")
    print("E(R" , x - 1 ,") = ", ''.join(expansion_res),sep='')

    xor_res = xor_key_e_ri(expansion_res, key_arr[x])
    print("\nXOR BETWEEN E(Ri) and The Key Phase : ")
    print("E(R" , x - 1 ,") = ", ''.join(expansion_res),sep='')
    print("K(" , x ,") = ", key_arr[x], sep='')
    print("---------------------------------------------------XOR")
    print("A(" , x ,") = ", xor_res, sep='')

    print("\nS-Box Phase : ")
    sbox_res = s_box_table_spliter(xor_res)
    print("B(" , x + 1 ,") = ", sbox_res,sep='')

    print("\nP-Box Permutation Phase : ")
    pbox_res = p_box_permutation(sbox_res)
    print("P(B" , x + 1 ,") = ", ''.join(pbox_res),sep='')

    print("\nLast XOR Phase : ")
    print("P(B" , x + 1 ,") = ", ''.join(pbox_res),sep='')
    print("L(" , x + 1 ,") = ", curr_li,sep='')
    print("---------------------------------------------------XOR")
    ri_res = xor_key_e_ri(pbox_res, curr_li)
    print("R(" , x + 1 ,") = ", ri_res,sep='')
    print("\n=============================================\n")

    curr_li = ''.join(curr_ri)
    curr_ri = ri_res
  
  print("L(" , iter ,") = ", curr_li)
  ri_li = curr_ri + curr_li
  print("R(" , iter ,")","L(" , iter ,") = ", ri_li)
  if iter >= 16:
    print("Cipher = ", ''.join(invers_initial_permutation(ri_li)))

else :
  hex_str = input("Input your hexa with space as separator : ")
  iter = int(input("How many iterations do you want : "))
  binary_str = convert_hex_to_binary(hex_str)
  key_back = iter
  for i in range(iter):
    key_back -= 1
    key = input("Input your binary key {} : ".format(key_back + 1))
    key_arr.append(key)
  ...
  for x in range(len(key_arr)):
    ...
    expansion_res = ekspansion_table(curr_ri)
    print("\nExpansion Phase : ")
    print("E(R" , iterationgs - 1,") = ", ''.join(expansion_res),sep='')

    xor_res = xor_key_e_ri(expansion_res, key_arr[x])
    print("\nXOR BETWEEN E(Ri) and The Key Phase : ")
    print("E(R" , iterationgs - 1 ,") = ", ''.join(expansion_res),sep='')
    print("K(" , iterationgs ,") = ", key_arr[x], sep='')
    print("---------------------------------------------------XOR")
    print("A(" , iterationgs ,") = ", xor_res, sep='')

    print("\nS-Box Phase : ")
    sbox_res = s_box_table_spliter(xor_res)
    print("B(" , iterationgs ,") = ", sbox_res,sep='')

    print("\nP-Box Permutation Phase : ")
    pbox_res = p_box_permutation(sbox_res)
    print("P(B" , iterationgs,") = ", ''.join(pbox_res),sep='')

    print("\nLast XOR Phase : ")
    print("P(B" , iterationgs,") = ", ''.join(pbox_res),sep='')
    print("L(" , iterationgs,") = ", curr_li,sep='')
    ri_res = xor_key_e_ri(pbox_res, curr_li)
    print("---------------------------------------------------XOR")
    print("R(" , iterationgs,") = ", ri_res,sep='')
    print("\n=============================================\n")
  ...
  print("L(" , iterationgs,") = ", curr_li, sep='')
  ri_li = curr_ri + curr_li
  print("R(" , iterationgs ,")","L(" , iterationgs ,") = ", ri_li, sep='')
  if iter >= 16:
    print("Plain = ", ''.join(invers_initial_permutation(ri_li)))
